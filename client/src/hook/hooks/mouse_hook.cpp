...
MouseHook::MouseHook()
    : Hook(std::string("mouse_hook"),
           std::string("48 8B C4 48 89 58 ?? 48 89 68 ?? 48 89 70 ?? 57 41 54 41 55 41 56 41 57 48 83 EC ?? 44 0F B7 BC 24")) {}

void MouseHook::HookFunc() { this->AutoHook(MouseHookCallback, &func_original); }
...
void MouseHook::MouseHookCallback(void *parm_1, char button, char state, short mouse_x, short mouse_y, short parm_6, short parm_7, char parm_8) {
    ...
    if (!block) return PLH::FnCast(func_original, MouseHookCallback)(parm_1, button, state, mouse_x, mouse_y, parm_6, parm_7, parm_8);
}
