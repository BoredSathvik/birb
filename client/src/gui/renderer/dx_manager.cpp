...
DxManager::DxManager()
{
    ...
    CoreDispatcher dispatcher = corewindow.get().Dispatcher();
...
void DxManager::CreateSwapChainResources()
{
    ...
    if (FAILED(d3d_device->CreateDepthStencilView(
            depth_stencil.get(),
            &depth_stencil_view_desc,
            d3d_depth_stencil_view.put())))
    {
        Logger::LogF("[Error] Failed to create depth_stencil_view");
        return;
    };

    screen_viewport = CD3D11_VIEWPORT(
        0.0f,
        0.0f,
        back_buffer_desc.Width,
        back_buffer_desc.Height);
...
void DxManager::OnWindowResize(CoreWindow const &, WindowSizeChangedEventArgs const &args)
{
    ...
    if (Logger::ErrorF(
            swapchain->ResizeBuffers(
                ...
                args.Size().Width * 96.0f / DisplayInformation::GetForCurrentView().LogicalDpi(),
                args.Size().Height * 96.0f / DisplayInformation::GetForCurrentView().LogicalDpi(),
